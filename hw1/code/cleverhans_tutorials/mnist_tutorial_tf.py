...
from cleverhans.dataset import FMNIST
from cleverhans.utils_tf import model_eval
from cleverhans.train import train
from cleverhans.attacks import FastGradientMethod, BasicIterativeMethod, SaliencyMapMethod, CarliniWagnerL2, MomentumIterativeMethod
...
def mnist_tutorial(train_start=0, train_end=60000, test_start=0,
  ...
  if clean_train:
    ...
    with tf.device("/gpu:0"):
      ...
      #preds =model.get_predicted_class(adv_x)
    ...
    for i in range(30):
      ...
      # for converting the one hot representation to label
      #true_label, adv_label = FMNIST.LABEL_NAMES[y_true_label], FMNIST.LABEL_NAMES[y_adv_label]
      

      plt.subplot(1,3,1)
      plt.title("Original : {}".format(y_true_label))
      plt.imshow(ori_images[:,:,0])

      plt.subplot(1,3,2)
      plt.title('Adversarial : {}'.format(y_adv_label))
      plt.imshow(adv_images[:,:,0])
  
      plt.subplot(1,3,3)
      plt.title('Difference')
      diff = abs(adv_images[:,:,0] - ori_images[:,:,0])
      plt.imshow(diff)
      l2dist = reduce_sum(tf.square(diff))
      l2dist=l2dist.eval(session=sess, feed_dict={x:x_train})
      print("...", l2dist)

      plt.show()

    percent_perturbed = np.mean(np.sum((adv_images[:,:,0] - ori_images[:,:,0])**2)**.5)
